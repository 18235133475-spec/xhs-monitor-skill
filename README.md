...
## v1.4.2 变更（2026-07-25）

- **修复撞墙后整轮空转**：撞详情墙时 XHS 会把路由 pushState 跳离主页，此后所有锚点点击失败、滚动无新卡（表现为 cards_seen 骤降 + 大面积「模态打开失败」）。新增 `on_profile_page`/`recover_profile`，检测到离开主页即中止本轮、回主页恢复后继续
- **失败卡不再重复撞墙**：本轮失败的 note_id 记入 `failed`，恢复重进后跳过，次日/下周补
- 每账号最多回主页恢复 2 次，仍失败则中止该账号并明示错误
